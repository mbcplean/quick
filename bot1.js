...
// In-memory pending requests and admin state
...
// Logging Helpers
// ---------------------------
// We'll accumulate logs for each wallet creation in a buffer.
let walletLogBuffer = [];
function appendLog(message) {
  walletLogBuffer.push(message);
  console.log(message);
}

// Helper: Send the accumulated wallet log to the user (in a code block)
async function sendWalletLog(chatId) {
  if (walletLogBuffer.length > 0) {
    const combined = walletLogBuffer.join("\n");
    await bot.sendMessage(chatId, "```\n" + combined + "\n```", { parse_mode: 'Markdown' });
    walletLogBuffer = []; // clear buffer
  }
}
...
async function createWallet() {
  ...
  appendLog(`> Creating wallet with address: ${wallet.address}`);
  return {
...
async function registerWallet(wallet, referralCode) {
  try {
    appendLog(`> Registering wallet: ${wallet.address}`);
    ...
    if (userInfoResponse.data.code === 200) {
      ...
      appendLog(`✅ Successfully registered wallet: ${wallet.address}`);
      return true;
    } else {
      appendLog(`❌ Failed to register wallet: ${wallet.address}`);
      return false;
    }
  } catch (error) {
    appendLog(`Error registering wallet ${wallet.address}: ${error.response?.data || error.message}`);
    return false;
  }
}
...
async function performChat(wallet, chatIndex) {
  try {
    if (!wallet.jwt) {
      appendLog(`No JWT available for wallet ${wallet.address}`);
      return false;
    }
    const axiosInstance = createAxiosInstance();
    const topic = getRandomTopic(wallet);
    appendLog(`Wallet ${wallet.address} - Chat ${chatIndex + 1}: "${topic}"`);
    ...
    if (chatResponse.status === 200) {
      wallet.chats++;
      appendLog(`✅ Chat ${chatIndex + 1} completed for wallet: ${wallet.address}`);
      return true;
    }
    return false;
  } catch (error) {
    appendLog(`Error during chat for wallet ${wallet.address}: ${error.response?.data || error.message}`);
    return false;
  }
}

async function completeDailyChats(wallet) {
  appendLog(`Starting daily chats for wallet: ${wallet.address}`);
  for (let i = 0; i < 5; i++) {
    const success = await performChat(wallet, i);
    if (!success) {
      appendLog(`Chat sequence stopped for wallet ${wallet.address} due to error`);
      break;
    }
    await new Promise(resolve => setTimeout(resolve, 2000 + Math.random() * 3000));
  }
  appendLog(`Completed ${wallet.chats}/5 chats for wallet: ${wallet.address}`);
}
...
async function processUserRequest(chatId, count, referralCode, topics) {
  try {
    ...
    for (let i = 0; i < count; i++) {
      // Clear log buffer for each wallet.
      walletLogBuffer = [];
      ...
      // Send combined log for this wallet to the user
      await sendWalletLog(chatId);
...
// ---------------------------
// TELEGRAM BOT FUNCTIONS & ADMIN PANEL
// ---------------------------
...
rlInterface.question('Enter your Telegram Bot Token: ', token => {
  rlInterface.question('Enter your Admin Telegram ID: ', adminIdInput => {
    ...
    bot.on('message', async msg => {
      ...
      if (!userState[chatId]) {
        // If no conversation state exists, ignore.
        return;
      }
